  ...
  const renderItems = (group: (typeof visualizerGroups)[0]) => (
    <div className="flex flex-wrap gap-4">
      {group.items.map((item, idx) => {
        const isDisabled = group.category === "soundWaves";
        return (
          <div
            key={idx}
            className={`w-16 h-16 rounded-lg flex items-center justify-center text-xs text-center ${
              isDisabled
            ...
            title={
              isDisabled ? "Not supported in export" : undefined
            }
          >
            {item.label}
            {isDisabled && (
